...
1. **Plugin hook** (`session-status`) fires on Claude Code lifecycle events and writes `.cstatus` JSON to `<profile>/projects/<encoded-path>/<session-id>.cstatus`. Installing it needs the `claude` CLI, which someone who only uses Claude Code inside the Claude desktop app does not have — the desktop app has Claude Code built in and ships no CLI. `PluginSetupView` hands those users the `/plugin marketplace add <bundled path>` and `/plugin install clawde@clawde-marketplace` steps to run in a session instead. Only the `SessionStart` hook starts the daemon — the `--signal` hooks write a `.csignal` for a daemon that must already be running — so a session open before the plugin was installed or updated never reports until it starts again (quit and reopen the Claude desktop app; `claude --resume` in a terminal). `/reload-plugins` registers the hooks but cannot help: that session's start is past. `PluginSetup.restartAdvice` is the one wording of this the app uses
